...
BEGIN_MESSAGE_MAP(CMFCApplicationView, CView)
	...
	ON_COMMAND(32789, &CMFCApplicationView::On32789)//水平压缩
	ON_COMMAND(32790, &CMFCApplicationView::On32790)//水平拉伸
	ON_COMMAND(32791, &CMFCApplicationView::On32791)//竖直压缩
	ON_COMMAND(32792, &CMFCApplicationView::On32792)//竖直拉伸
...
void CMFCApplicationView::OnLButtonDown(UINT nFlags, CPoint point)
{
	...
	if (isFly == false) {
		...
		if (nPosition < 0) {
			...
			pDC->TextOutW((pTemp.x - x0) * nClientRect.right  / (x - x0) * 0.8 + 0.1 * nClientRect.right + 5,
				          (pTemp.y - y0) * nClientRect.bottom / (y - y0) * 0.8 + 0.1 * nClientRect.bottom, str);
		...
		pDC->TextOutW((v[nPosition].x - x0) * nClientRect.right  / (x - x0) * 0.8 + 0.1 * nClientRect.right + 5,
			          (v[nPosition].y - y0) * nClientRect.bottom / (y - y0) * 0.8 + 0.1 * nClientRect.bottom, str);
...
void CMFCApplicationView::On32789()
{
	// TODO: 在此添加命令处理程序代码
	double length = x - x0;
	x0 = x0 - length * 0.2;
...
void CMFCApplicationView::On32790()
{
	// TODO: 在此添加命令处理程序代码
	double length = x - x0;
	x0 = x0 + length * 0.2;
...
void CMFCApplicationView::On32791()
{
	// TODO: 在此添加命令处理程序代码
	double length = y - y0;
	y0 = y0 - length * 0.2;
...
void CMFCApplicationView::On32792()
{
	// TODO: 在此添加命令处理程序代码
	double length = y - y0;
	y0 = y0 + length * 0.2;
